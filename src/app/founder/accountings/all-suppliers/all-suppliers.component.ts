...
import { SupplierFormDialogComponent } from './dialogs/form-dialog/form-dialog.component';/* 
import { AboutSupplierComponent } from './dialogs/about-supplier/about-supplier.component'; */
import { SupplierDeleteDialogComponent } from './dialogs/delete/delete.component';
  ...
  implements OnInit
{
  ...
  exampleDatabase?: SuppliersService;
  ...
  addNew() {
    this.router.navigate([
      "/founder/accountings/all-suppliers/add-suppliers"
    ]);
  }
  ...
  editCall(row: Users) {
    ...
    const dialogRef = this.dialog.open(SupplierFormDialogComponent, {
      data: { suppliers: row, action: 'edit', },
      ...
      if (result === 1) {
        ...
        if (foundIndex != null && this.exampleDatabase) {
          this.exampleDatabase.dataChange.value[foundIndex] =
            this.suppliersService.getDialogData();
  ...
  About(row: Users) {
    ...
    const dialogRef = this.dialog.open(AboutSupplierComponent, {
      data: { suppliers: row,},
      direction: tempDirection,
    });
  }
  deleteItem(row: Users) {
    ...
    const dialogRef = this.dialog.open(SupplierDeleteDialogComponent, {
      data: row,
      direction: tempDirection,
    });
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
  ...
  removeSelectedRows() {
    ...
    for( let i of this.selection.selected){
      this.suppliersService.deleteUsers(i.id);
      this.ngOnInit();
      this.refreshTable();
  ...
  public loadData() {
    this.exampleDatabase = new SuppliersService(
  ...
  exportExcel() {
    ...
    TableExportUtil.exportToExcel(exportData, 'Liste du Personnel');
  }
...
export class ExampleDataSource extends DataSource<Users> {
  ...
  meta!: any;
  constructor(
    public exampleDatabase: SuppliersService,
  ...
  connect(data: any): Observable<Users[]> {
    ...
    this.exampleDatabase.getAllUserss(data, this.paginator.pageIndex, this.paginator.pageSize);
