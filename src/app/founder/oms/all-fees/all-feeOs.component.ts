...
//import { Fee0sService } from './oms.service';
...
import { Fee0sService } from './feeOs.service';
...
@Component({
  selector: 'app-all-feeOs',
  templateUrl: './all-feeOs.component.html',
  styleUrls: ['./all-feeOs.component.scss'],
})
export class AllFee0sComponent
  ...
  implements OnInit
{
  ...
  constructor(
    ...
    public feeOsService: Fee0sService,
  ...
  public loadData() {
    this.s = false
    this.exampleDatabase = new Fee0sService(
      this.httpClient,
      this.router,
      this.authService,
...
export class ExampleDataSource extends DataSource<Transactions> {
  ...
  constructor(
    public exampleDatabase: Fee0sService,
  ...
  connect(): Observable<Transactions[]> {
    ...
    this.exampleDatabase.getAllFee0ss();
    return merge(...displayDataChanges).pipe(
      map(() => {
        // Filter data
        this.filteredData = this.exampleDatabase.data
          .slice()
          .filter((feeOs: Transactions) => {
            const searchStr = (
              feeOs.student.name +
              feeOs.payment_mode +
              feeOs.advancePayment +
              feeOs.created_at +
              feeOs.fee.name
            ).toLowerCase();
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case 'id':
          ...
          break;
        case 'fee.name':
          [propertyA, propertyB] = [a.fee.name, b.fee.name];
          break;
        case 'payment_mode':
