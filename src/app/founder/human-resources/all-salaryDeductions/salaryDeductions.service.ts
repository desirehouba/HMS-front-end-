...
@Injectable()
export class SalaryDeductionsService extends UnsubscribeOnDestroyAdapter {
  private readonly API_URL = 'assets/data/invoices.json';
  isTblLoading = true;
  dataChange: BehaviorSubject<SalaryDeductions[]> = new BehaviorSubject<SalaryDeductions[]>([]);
  metaChange: BehaviorSubject<any[]> = new BehaviorSubject<any[]>([]);
  // Temporarily stores data from dialogs
  dialogData!: SalaryDeductions;
  paylaod: any = {};
  constructor(
  ...
  getAllSalaryDeductionss(data: any, page_items : number, nbre_items: number) {
    ...
    this.subs.sink = this.httpClient.post<any>(
      `${environment.apiUrl}/salaries-deductionsall`, this.paylaod)
      .subscribe({
        next: (data) => {
  ...
  deleteSalaryDeductions(id: number): void {
