...
import { FormDialogRoomComponent } from './dialogs/form-dialog/form-dialog.component';
import { AboutRoomComponent } from './dialogs/about-room/about-room.component';
  ...
  implements OnInit
{
  ...
  addNew() {
    this.router.navigate([
      "/founder/hosting/all-rooms/add-room"]);
  }
  ...
  setStatus() {
    if (this.f["status"].value != null) {
      localStorage.setItem('statusRoom', JSON.stringify(this.f["status"].value));
    } else {
      localStorage.setItem('statusRoom', JSON.stringify(null));
    }
    this.ngOnInit();
  } 
  ...
  editCall(row: Rooms) {
    ...
    const dialogRef = this.dialog.open(FormDialogRoomComponent, {
      data: {
        rooms: row,
        action: 'edit',
      },
      ...
      if (result === 1) {
        ...
        if (foundIndex != null && this.exampleDatabase) {
          this.exampleDatabase.dataChange.value[foundIndex] =
            this.roomsService.getDialogData();
  ...
  About(row: Rooms) {
    ...
    const dialogRef = this.dialog.open(AboutRoomComponent, {
      data: { rooms: row, },
      direction: tempDirection,
    });
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {});
  }
  deleteItem(row: Rooms) {
    this.id = row.id;
    let tempDirection: Direction;
    if (localStorage.getItem('isRtl') === 'true') {
      tempDirection = 'rtl';
  ...
  removeSelectedRows() {
    ...
    this.selection.selected.forEach((item) => {
      ...
      this.refreshTable();
      this.selection = new SelectionModel<Rooms>(true, []);
    });
    
    for( let i of this.selection.selected){
      this.roomsService.deleteRooms(i.id);
      this.ngOnInit();
      this.refreshTable();
  ...
  public loadData() {
    this.exampleDatabase = new RoomsService(
  ...
  // context menu
  onContextMenu(event: MouseEvent, item: Rooms) {
...
export class ExampleDataSource extends DataSource<Rooms> {
  ...
  connect(data: any): Observable<Rooms[]> {
    ...
    this.exampleDatabase.getAllRoomss(data, this.paginator.pageIndex, this.paginator.pageSize);
