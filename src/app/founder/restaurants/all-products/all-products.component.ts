...
import { ProductsDialogComponent } from './dialogs/form-dialog/form-dialog.component'; 
import { ProductsDeleteDialogComponent } from './dialogs/delete/delete.component';
...
import { AboutProductComponent } from './dialogs/about-product/about-product.component';
  ...
  implements OnInit
{
  ...
  permissions!: any[]
  classrooms: any [] = []
  idclasse = this.authService.currentUserValue.idClasse
  vide = {
    id: null,
    name : 'Tous / All' 
  }
  ...
  addNew() {
    this.router.navigate([
      "/founder/restaurants/all-products/add-product"
    ]);
  }
      ...
      if (result === 1) {
        ...
        if (foundIndex != null && this.exampleDatabase) {
          this.exampleDatabase.dataChange.value[foundIndex] =
            this.productsService.getDialogData();
  ...
  About(row: Products) {
    ...
    const dialogRef = this.dialog.open(AboutProductComponent, {
      data: { products: row, },
      direction: tempDirection,
    });
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {});
  }
  deleteItem(row: Products) {
    this.id = row.id;
    let tempDirection: Direction;
    if (localStorage.getItem('isRtl') === 'true') {
      tempDirection = 'rtl';
    ...
    const dialogRef = this.dialog.open(ProductsDeleteDialogComponent, {
      data: row,
      direction: tempDirection,
    });
    this.subs.sink = dialogRef.afterClosed().subscribe((result) => {
  ...
  removeSelectedRows() {
    ...
    for( let i of this.selection.selected){
      this.productsService.deleteProducts(i.id);
      this.ngOnInit();
      this.refreshTable();
  ...
  public loadData() {
    this.exampleDatabase = new ProductsService(
  ...
  // context menu
  onContextMenu(event: MouseEvent, item: Products) {
...
export class ExampleDataSource extends DataSource<Products> {
  ...
  meta!: any;
  constructor(
    public exampleDatabase: ProductsService,
  ...
  connect(data: any): Observable<Products[]> {
    ...
    this.exampleDatabase.getAllProductss(
